...
<details>
<summary><b>Ligation metadata CSV format</b></summary>

The ligation path requires `demultiplex.metadata`; a missing value raises at runtime. The CSV must have a header row with the columns `eventID` (sample id), `tag_demultiplex` (tag sequence), and `library` (library id). The loader auto-detects comma- or semicolon-separated files.

A lab `Corr_tags` file does not work as-is: it is headerless with column order `well;library;sample;project;marker;tagseq`, so convert it to a headered CSV first, mapping `eventID=sample`, `tag_demultiplex=tagseq`, `library=library`.
...
Two-pass trimming handles both primer orientations. Pass 1 temporary files are deleted after pass 2. When `trimming.discard_untrimmed: true`, reads without a detected 5' primer are dropped in pass 1.

<details>
<summary><b>Two-pass algorithm (5' then 3')</b></summary>
...
<details>
<summary><b>The 7-step SWARM workflow</b></summary>
...
`cleaning.mode` defaults to `flag`, which adds the `in_negative_control` annotation without changing any counts.

> [!WARNING]
> `subtract` zeroes a flagged feature's reads in the samples its driving control covers (extraction blanks clean their own extraction batch; PCR blanks clean the whole dataset). Subtraction is high-consequence and stays opt-in.
