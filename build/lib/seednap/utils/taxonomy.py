...
# Backwards-compatible alias (the taxonomy post-processor and its tests refer to
# the rank list under this name).
DEFAULT_RANK_COLUMNS: Tuple[str, ...] = TAXONOMIC_RANKS

# Canonical missing-taxonomy sentinels: the COMPLETE union of the variants that
# used to be scattered across modules (BLAST's ("", "na", "nan"); the
# post-processor's {"", "NA", "None", "nan", "NaN", "Na"}; the HTML report's
# {"Unassigned", "unassigned", "", "NA", "nan", "None"}). Stored lowercased and
# matched case-insensitively via `is_missing_taxon`, so matching is consistent
# everywhere and no call site mis-classifies a value relative to its old set
# (every previously-matched literal still matches after strip()+lower()). No
# real taxon name normalises to any of these.
MISSING_TAXON_VALUES: frozenset = frozenset({"", "na", "nan", "none", "unassigned"})

UNASSIGNED_LABEL = "Unassigned"
CONTAMINANT_FLAG_COL = "is_contaminant_candidate"


def is_missing_taxon(value: object) -> bool:
    ...
    try:
        ...
        pass
    return str(value).strip().lower() in MISSING_TAXON_VALUES
...
def _normalize_unassigned(series: pd.Series, unassigned_label: str = UNASSIGNED_LABEL) -> pd.Series:
    ...
    s = s.astype(str)
    is_missing = s.str.strip().str.lower().isin(MISSING_TAXON_VALUES)
    return s.where(~is_missing, unassigned_label)
